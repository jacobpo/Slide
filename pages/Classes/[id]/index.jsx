...
function Sidebar(props) {
  ...
  useEffect(() => {
    ...
    if (window.innerWidth < 600) {
      ...
      setCSS(
        ' flex flex-col items-center  h-screen bg-[#8b9dc3] w-40  text-center text-white space-y-8 w-screen',
      )
    }
    // } else {
    // setCSS(
    //   ' flex flex-col items-center  h-screen bg-[#8b9dc3] w-40  text-center text-white space-y-8',
    // )
                ...
                {groups?.map(function (group, index) {
                  return (
                    <li key={index} className="mb-[5px]">
                      <a href={`/Classes${id}/studygroups/${group.id}`}>
                        {' '}
                        {group.Name}
                      </a>
                    </li>
